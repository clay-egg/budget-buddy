...
import { Link } from 'react-router-dom' // Import Link for potential edit functionality
...
function ExpensesList() {
  ...
  const handleDelete = async (id: string) => {
    // Use the ESLint disable comment to allow the confirm() function
    // eslint-disable-next-line no-restricted-globals
    if (!confirm('Are you sure you want to delete this expense?')) return // User cancelled

    setDeleteLoading(id) // Set loading state for the specific item being deleted
    ...
    try {
      ...
      if (deleteError) throw deleteError // Throw error if deletion fails

      // Remove the expense from the local state
      setExpenses(prev => prev.filter(expense => expense.id !== id)) 
    } catch (err: any) {
      setError(err.message || 'Failed to delete expense') // Set error message
    } finally {
      setDeleteLoading(null) // Clear loading state for the item
    }
  }
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="ml-4 flex items-center space-x-2">
                  ...
                  <button
                    ...
                    disabled={deleteLoading === expense.id} // Disable button while deleting
                    className="p-2 text-gray-400 hover:text-red-600 rounded-full hover:bg-red-50 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-red-500"
